    ...
    companion object {
        ...
        val MIGRATION_1_2 = object : Migration(1, 2) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("""CREATE TABLE IF NOT EXISTS `new_nyaaReleasePreview` (`name` TEXT NOT NULL, `magnet` TEXT NOT NULL, `timestamp` INTEGER NOT NULL, `seeders` INTEGER NOT NULL, `leechers` INTEGER NOT NULL, `completed` INTEGER NOT NULL, `category` TEXT NOT NULL, `releaseSize` TEXT NOT NULL, `number` INTEGER NOT NULL, `dataSource` INTEGER NOT NULL, PRIMARY KEY(`number`, `dataSource`));
                """)
                database.execSQL("""INSERT INTO new_nyaaReleasePreview(dataSource, number, name, magnet, timestamp, seeders, leechers, completed, category, releaseSize) SELECT 0, id, name, magnet, timestamp, seeders, leechers, completed, category, releaseSize FROM nyaaReleasePreview;""")
                ...
                database.execSQL("ALTER TABLE subscribedTracker ADD COLUMN `dataSource` INTEGER NOT NULL DEFAULT 0")
            }
        }
